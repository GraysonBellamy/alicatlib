#!/usr/bin/env python3
"""Open a serial port, poll once, print the reading.

The "does it work" smoke test — exercises port open, the context-manager
lifecycle, and typed :class:`Reading` access. Works against any Alicat
flow or pressure device.
...
def main() -> None:
    ...
    with Alicat.open(port, serial=SerialSettings(port=port, baudrate=baud)) as dev:
        ...
        print(f"received_at: {reading.received_at.isoformat()}")
        print()
        ...
        for label, stat in (
            ...
            value = reading.get_statistic(stat)
            if value is None:
                continue
